...
@section('title', ($isEdit ?? false) ? 'تعديل بيانات المكتب | آمر تم' : 'إضافة مكتب/مستشار | آمر تم')

@section('dashboard-content')

@php
    // نفس حساب $persona / $dashboardMenu الموجود في admin.layout.blade.php —
    // بدونها يظهر شريط جانبي/ترويسة فارغة لأن layouts.dashboard لا يملك View Composer.
    $modeValue = $mode ?? 'office';
    $modeClient = false; // غير مستخدم في سياق الأدمن
    ...
    $pageTitle = ($isEdit ? 'تعديل بيانات المكتب' : 'إضافة مكتب / مستشار') . ' — ' . $persona['label'];
    $dashboardMenu = \App\Support\DashboardRegistry::menuFor($frontTypes, 'admin');
@endphp

<div class="min-h-screen bg-gray-50 py-6" style="direction:rtl" dir="rtl">
    <div class="mx-auto max-w-5xl px-4">

        <div class="mb-5 flex items-center justify-between">
            <div>
                <a href="{{ route('amrtm.admin.dashboard') }}#offices"
                    class="inline-flex items-center gap-1.5 text-[12px] font-bold text-slate-500 hover:text-[#0f766e]">
                    <i class="ti ti-arrow-right"></i>
                    الرجوع إلى إدارة المكاتب
                </a>
                <h1 class="mt-2 text-[19px] font-extrabold text-[#172033]">
                    {{ $isEdit ? 'تعديل بيانات المكتب' : 'إضافة مكتب / مستشار جديد' }}
                </h1>
                <p class="mt-1 text-[12px] text-slate-500">
                    نفس نموذج التسجيل الذاتي بالضبط — تُنشأ/تُعدَّل البيانات كما لو أن المكتب سجَّل نفسه.
                </p>
            </div>
        </div>

        @if ($errors->any())
            <div class="mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-[12px] text-red-700">
                <div class="mb-1 font-extrabold"><i class="ti ti-alert-circle ml-1"></i>يرجى تصحيح الأخطاء التالية:</div>
        ...
        @if (session('success'))
            <div class="mb-4 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-[12px] text-emerald-700">
                {{ session('success') }}
            </div>
        @endif

        {{-- ===================== GENERAL ERRORS (تُملأ من JS دون إعادة تحميل الصفحة) ===================== --}}
        <div id="provider-form-errors" role="alert"
            class="mb-4 hidden flex items-start gap-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-[12px] leading-relaxed text-red-900">
        ...
        <form method="POST"
            action="{{ $isEdit ? route('amrtm.admin.offices.update', $office->id) : route('amrtm.api.admin.offices.create') }}"
            ...
            <section
                ...
                {{-- نوع الحساب: مكتب مساند / مستشار / مساند + استشاري (موحّد) --}}
                ...
                <div class="mb-5 border-b border-slate-100 pb-5">
                    ...
                    <p class="mb-3 -mt-1 text-[11px] leading-relaxed text-slate-500">
                        ...
                        اختر نوع الكيان التجاري. في خيار «مساند + استشاري» تعمل المنشأة بالنشاطين معاً في لوحة تحكم موحّدة.
                    </p>
                    <div class="flex w-full max-w-4xl flex-col gap-2.5 sm:flex-row sm:gap-3" style="direction:rtl">
                        <button type="button" id="sub-commission"
                            class="sub-type-pill flex flex-1 cursor-pointer items-center justify-start gap-2.5 rounded-xl border px-4 py-3 text-right transition-all duration-200 {{ !$modeConsultant && !$modeMixed ? 'border-[#0f766e] bg-teal-50 text-[#0f766e] shadow-[0_4px_14px_rgba(15,118,110,.12)]' : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300 hover:bg-slate-50' }}"
                            onclick="setSubscriptionType('commission')">
                            <i class="ti ti-building text-[18px]"></i>
                            <span class="text-[12px] font-extrabold">
                                مكتب مساند
                                <small class="block text-[10px] font-semibold opacity-80">عمولة لكل عملية</small>
                            </span>
                        </button>
                ...
                {{-- حقول المكتب/المنشأة — مُكوَّن موجود يُستدعى دون تعديل --}}
                @include('partials.public.provider-office-fields')

            </section>

            {{-- التخصص + الخدمات + الشعار + المستندات — مكوّن موجود يُستدعى دون تعديل --}}
            ...
            <section
                ...
                <div class="flex flex-col items-center justify-between gap-5 sm:flex-row">
                    <div class="text-[11px] leading-relaxed text-slate-500">
                        <i class="ti ti-info-circle ml-1 text-[#0f766e]"></i>
                        خانات علامة <span class="text-red-600">*</span> إلزامية.<br>
                        عند التعديل: كلمة المرور والمستندات اختيارية — تُستبدل فقط إذا أُرفقت ملفات جديدة.
                    </div>
                    <div class="flex items-center gap-3">
                        <a href="{{ route('amrtm.admin.dashboard') }}#offices"
                            class="inline-flex min-h-[45px] items-center justify-center gap-2 whitespace-nowrap rounded-xl border border-slate-300 bg-white px-5 py-2.5 text-[12px] font-extrabold text-slate-600 transition-colors duration-200 hover:bg-slate-50">
                            <i class="ti ti-x"></i>
                            إلغاء
                        </a>
                        <button type="submit" id="submit-btn"
                            ...
                            <i class="ti {{ $isEdit ? 'ti-check' : 'ti-send' }}"></i>
                            {{ $isEdit ? 'حفظ التعديلات' : 'حفظ المكتب' }}
                        </button>
                    </div>
                </div>
            </section>
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const isEditMode = @json((bool)($isEdit ?? false));
            const submitLabel = isEditMode ? 'حفظ التعديلات' : 'حفظ المكتب';

            /* =====================================================
               نوع الاشتراك (مكتب مساند / مستشار / مزدوج)
            ===================================================== */
            ...
            window.setSubscriptionType = function (value) {
                ...
                if (subCommission) {
                    subCommission.className = 'sub-type-pill flex flex-1 cursor-pointer items-center justify-start gap-2.5 rounded-xl border px-4 py-3 text-right transition-all duration-200 ' + (!isSubscription && !isMixed ? active : inactive);
                }
                if (subSubscription) {
                    subSubscription.className = 'sub-type-pill flex flex-1 cursor-pointer items-center justify-start gap-2.5 rounded-xl border px-4 py-3 text-right transition-all duration-200 ' + (isSubscription && !isMixed ? active : inactive);
                }
                if (subMixed) {
...
${isChecked && svcFields.length ? `<span class="inline-flex items-center gap-1 rounded bg-violet-50 px-1.5 py-0.5 text-[11px] font-semibold text-violet-800">${svcFields.length} حقول مخصصة</span>` : ''}
                                    ...
                                    <div class="service-card-desc mt-1 text-[11px] leading-relaxed text-slate-500">${svc.requirements || ''}</div>
                                </div>
                            </div>
${isChecked ? `
            ...
            /* =====================================================
               EDIT MODE ADAPTERS
               (الخادم يسمح في التعديل بكلمة مرور/ملفات اختيارية بينما
               partials الحالية تعرضها required — نُزيل الشرط عبر JS)
            ===================================================== */

            if (isEditMode) {
                ['password', 'password_confirmation', 'commercial_register_image', 'license_image', 'cv', 'trademark_certificate'].forEach(function (id) {
                    const field = document.getElementById(id);
                    if (field) {
                        field.removeAttribute('required');
                        if (id === 'password' || id === 'password_confirmation') {
                            field.setAttribute('placeholder', id === 'password' ? 'اتركه فارغاً للإبقاء على الحالي' : 'أعد إدخال كلمة المرور الجديدة');
                        }
                    }
                });
            }
            ...
            if (isEditMode) {
                ...
                if (officeType) officeType.value = oldOfficeType;
                loadSpecialties(oldSpecialty || '');
            }
            ...
            form.addEventListener('submit', function (event) {
                ...
                submitButton.innerHTML = '<i class="ti ti-loader animate-spin"></i> جاري الحفظ...';
                ...
                fetch(form.action, {
                    ...
                    .then(async function (response) {
                        ...
                        if (!response.ok) {
                            ...
                            if (data && data.errors) {
                                ...
                                serverMessages.push(data && data.message ? data.message : 'تعذر الحفظ. يرجى مراجعة البيانات ثم المحاولة مرة أخرى.');
                            }
                    ...
                    .finally(function () {
                        ...
                        submitButton.innerHTML = '<i class="ti ti-send"></i> ' + submitLabel;
                    });
            });
        });
